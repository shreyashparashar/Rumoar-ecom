...
export function CartProvider({ children }) {
  ...
  const add = useCallback((id, colour, qty = 1) => dispatch({ type: "add", id, colour, qty }), []);
  const setQty = useCallback((id, colour, qty) => dispatch({ type: "qty", id, colour, qty }), []);
  const remove = useCallback((id, colour) => dispatch({ type: "remove", id, colour }), []);
  const clear = useCallback(() => dispatch({ type: "clear" }), []);

  const value = useMemo(() => ({
    lines: detailed, count, subtotal, shipping, total,
    add, setQty, remove, clear, freeOver: SHIP_FREE_OVER,
  }), [detailed, count, subtotal, shipping, total, add, setQty, remove, clear]);
